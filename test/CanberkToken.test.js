...
// Start test block
describe('CanberkToken', function () {
  before(async function () {
    this.CanberkToken = await ethers.getContractFactory('CanberkToken');
  });

  beforeEach(async function () {
    this.canberkToken = await this.CanberkToken.deploy();
    await this.canberkToken.deployed();

    this.decimals = await this.canberkToken.decimals();
    ...
    this.signerContract = this.canberkToken.connect(signers[1]);
  });

  // Test cases
  it('Creates a token with a name', async function () {
    expect(await this.canberkToken.name()).to.exist;
    // expect(await this.canberkToken.name()).to.equal('CanberkToken');
  });

  it('Creates a token with a symbol', async function () {
    expect(await this.canberkToken.symbol()).to.exist;
    // expect(await this.canberkToken.symbol()).to.equal('FUN');
  });

  it('Has a valid decimal', async function () {
    expect((await this.canberkToken.decimals()).toString()).to.equal('18');
  })

  it('Has a valid total supply', async function () {
    const expectedSupply = ethers.utils.parseUnits('1000000', this.decimals);
    expect((await this.canberkToken.totalSupply()).toString()).to.equal(expectedSupply);
  });

  it('Is able to query account balances', async function () {
    const ownerBalance = await this.canberkToken.balanceOf(this.ownerAddress);
    expect(await this.canberkToken.balanceOf(this.ownerAddress)).to.equal(ownerBalance);
  });

  it('Transfers the right amount of tokens to/from an account', async function () {
    const transferAmount = 1000;
    await expect(this.canberkToken.transfer(this.recipientAddress, transferAmount)).to.changeTokenBalances(
        this.canberkToken,
  ...
  it('Emits a transfer event with the right arguments', async function () {
    ...
    await expect(this.canberkToken.transfer(this.recipientAddress, ethers.utils.parseUnits(transferAmount.toString(), this.decimals)))
        .to.emit(this.canberkToken, "Transfer")
  ...
  it('Allows for allowance approvals and queries', async function () {
    ...
    expect((await this.canberkToken.allowance(this.recipientAddress, this.ownerAddress))).to.equal(ethers.utils.parseUnits(approveAmount.toString(), this.decimals));
  });
  ...
  it('Emits an approval event with the right arguments', async function () {
    ...
    await expect(this.signerContract.approve(this.ownerAddress, ethers.utils.parseUnits(approveAmount.toString(), this.decimals)))
        .to.emit(this.canberkToken, "Approval")
  ...
  it('Allows an approved spender to transfer from owner', async function () {
    ...
    await this.canberkToken.transfer(this.recipientAddress, ethers.utils.parseUnits(transferAmount.toString(), this.decimals))
    await this.signerContract.approve(this.ownerAddress, ethers.utils.parseUnits(transferAmount.toString(), this.decimals))
    await expect(this.canberkToken.transferFrom(this.recipientAddress, this.ownerAddress, transferAmount)).to.changeTokenBalances(
        this.canberkToken,
        [this.ownerAddress, this.recipientAddress],
        [transferAmount, -transferAmount]
      );
  ...
  it('Emits a transfer event with the right arguments when conducting an approved transfer', async function () {
    ...
    await this.canberkToken.transfer(this.recipientAddress, ethers.utils.parseUnits(transferAmount.toString(), this.decimals))
    await this.signerContract.approve(this.ownerAddress, ethers.utils.parseUnits(transferAmount.toString(), this.decimals))
    await expect(this.canberkToken.transferFrom(this.recipientAddress, this.ownerAddress, ethers.utils.parseUnits(transferAmount.toString(), this.decimals)))
        .to.emit(this.canberkToken, "Transfer")
  ...
  it('Allows allowance to be increased and queried', async function () {
    ...
    const previousAllowance = await this.canberkToken.allowance(this.recipientAddress, this.ownerAddress);
    ...
    expect((await this.canberkToken.allowance(this.recipientAddress, this.ownerAddress))).to.equal(expectedAllowance);
  });
